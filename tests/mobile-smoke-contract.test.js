...
test('mobile smoke command and workflow are present and pinned', () => {
  ...
  assert.equal(packageJson.scripts['public-pages-smoke'], 'node tests/public-pages-smoke.cjs');
  assert.equal(packageJson.scripts['validate-inline-js'], 'node scripts/validate-inline-js.js');
  ...
  assert.match(workflow, /npm run public-pages-smoke/);
  assert.match(workflow, /npm run validate-inline-js/);
