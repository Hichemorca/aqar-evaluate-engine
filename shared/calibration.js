    ...
    function renderShadowSection(propertyType, propertyConfig) {
      ...
      return `<div class="section"><div class="section-title"><span>Experimental Shadow Controls</span><span class="panel-note">disabled by default · neutral = 1.00×</span></div><div class="small" style="margin-bottom:10px">These controls affect only the experimental shadow preview until separately approved for production.</div><div class="field-card-grid">${projectCard}${ratioCard}${renovationCard}${globalCard}</div></div>`;
    }
    ...
    function renderInputField(path, value, options = {}) {
      ...
      return `<div class="field"><label for="${escapeHtml(path.join('__'))}">${escapeHtml(label(path.slice(-2)))}</label><div style="display:flex;align-items:center;gap:5px"><input id="${escapeHtml(path.join('__'))}" data-path="${escapeHtml(path.join('.'))}" data-percent="${Boolean(options.percent)}" type="number" step="${step}" value="${displayValue}">${suffix}</div></div>`;
    }
    ...
    function renderFieldCard(title, groups, cardClass = '') {
      ...
      const body = visibleGroups.map(group => `${group.heading ? `<div class="panel-note" style="margin:8px 0 6px">${escapeHtml(group.heading)}</div>` : ''}<div class="field-grid">${group.fields.join('')}</div>`).join('');
    ...
    function addProjectMultiplierRow(propertyType) {
      ...
      const wrapper = document.createElement('div'); wrapper.className = 'field-grid'; wrapper.style.cssText = 'grid-template-columns:2fr 1fr auto;align-items:end;margin-bottom:8px';
